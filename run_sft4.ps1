# Detached launcher for SFT-4: ONE finetune, no DPO half.
#
#   SFT-4: finetune from models/enigma_v2_238m_facts/model.pth -- the same
#   facts checkpoint SFT-2 came from -- on the current data/sft/mix.jsonl.
#   -> models/enigma_v2_sft4
#
...
if ($existing) {
    "REFUSED: training already running (PID $($existing.ProcessId)) at $(Get-Date -Format o)" |
        Set-Content -Path (Join-Path $repo "train_sft4.refused") -Encoding utf8
    exit 1
}
